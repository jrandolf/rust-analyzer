...
pub fn test() {
    ...
    let _result = generator.next();
        ...
        expect![[r#"
            41..136 '{     ...t(); }': ()
            51..64 'mut generator': {unknown}
            67..97 'gen { ...     }': {unknown}
            107..114 '_result': {unknown}
            117..126 'generator': {unknown}
            117..133 'genera...next()': {unknown}
        "#]],
    );
}
...
pub fn test(mut context: Context) {
    ...
    let _result = Pin::new(&mut async_generator).poll_next(&mut context);
        ...
        "#,
        expect![[r#"
            103..114 'mut context': Context<'?>
            125..270 '{     ...xt); }': ()
            135..154 'mut as...erator': impl Fn() -> impl Future<Output = {unknown}>
            157..162 'async': impl Fn() -> impl Future<Output = {unknown}>
            163..193 'gen { ...     }': {unknown}
            203..210 '_result': {unknown}
            213..221 'Pin::new': fn new<&'? mut impl Fn() -> impl Future<Output = {unknown}>>(&'? mut impl Fn() -> impl Future<Output = {unknown}>) -> Pin<&'? mut impl Fn() -> impl Future<Output = {unknown}>>
            213..243 'Pin::n...rator)': Pin<&'? mut impl Fn() -> impl Future<Output = {unknown}>>
            213..267 'Pin::n...ntext)': {unknown}
            222..242 '&mut a...erator': &'? mut impl Fn() -> impl Future<Output = {unknown}>
            227..242 'async_generator': impl Fn() -> impl Future<Output = {unknown}>
            254..266 '&mut context': &'? mut Context<'?>
            259..266 'context': Context<'?>
        "#]],
    );
}
...
"#,
        expect![[r#"
            99..110 'mut context': Context<'?>
            121..339 '{     ...   } }': ()
            131..136 'mut g': impl Fn() -> impl Future<Output = {unknown}>
            139..144 'async': impl Fn() -> impl Future<Output = {unknown}>
            145..180 'gen { ...     }': {unknown}
            187..337 'match ...     }': ()
            193..201 'Pin::new': fn new<&'? mut impl Fn() -> impl Future<Output = {unknown}>>(&'? mut impl Fn() -> impl Future<Output = {unknown}>) -> Pin<&'? mut impl Fn() -> impl Future<Output = {unknown}>>
            193..209 'Pin::n...mut g)': Pin<&'? mut impl Fn() -> impl Future<Output = {unknown}>>
            193..233 'Pin::n...ntext)': Poll<Option<()>>
            202..208 '&mut g': &'? mut impl Fn() -> impl Future<Output = {unknown}>
            207..208 'g': impl Fn() -> impl Future<Output = {unknown}>
            220..232 '&mut context': &'? mut Context<'?>
